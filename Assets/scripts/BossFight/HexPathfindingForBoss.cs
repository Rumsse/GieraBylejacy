...
public class HexPathfindingForBoss : MonoBehaviour
{
    ...
    [SerializeField] private Transform bossUnit;     // Jednostka przeciwnika
    ...
    private void Start()
    {
        ...
        Vector3Int startEnemyPos = hexTilemap.WorldToCell(bossUnit.position);  // Pozycja startowa przeciwnika
        ...
        bossMovement.SetPath(path);  // Inicjalizowanie œcie¿ki w EnemyMovement
    }
    ...
    private void Update()
    {
        if (bossUnit != null)
        {
            ...
            Vector3Int currentEnemyPos = hexTilemap.WorldToCell(bossUnit.transform.position);
            ...
            if (bossMovement.isEnemyMoving && path.Count > 0) // Przeciwnik porusza siê po œcie¿ce, jeœli jest czas na jego ruch
            {
                bossMovement.MoveEnemyAlongPath();
            }
